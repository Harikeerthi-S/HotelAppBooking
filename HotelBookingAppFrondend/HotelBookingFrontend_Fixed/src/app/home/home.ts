...
export class Home {
  ...
  availableAmenities = computed(() => this.amenities().filter(a => a.isAvailable));
  ...
  constructor() {
    ...
    // Only load amenities if user is logged in to avoid 403 errors
    if (this.isLoggedIn()) {
      this.apiService.apiGetAmenities().subscribe({ 
        next: res => this.amenities.set(res || []), 
        error: () => {
          console.log('Amenities unavailable - user not authenticated');
          this.amenities.set([]);
        }
      });
    }
    ...
    $userStatus.subscribe({
      next: u => {
        if (u.userId > 0 && u.role === 'user') {
          this.loadUpcomingBookings(u.userId);
        }
        // Load amenities when user logs in
        if (u.userId > 0 && this.amenities().length === 0) {
          this.apiService.apiGetAmenities().subscribe({ 
            next: res => this.amenities.set(res || []), 
      ...
      error: () => {
        console.log('User status subscription error - continuing with default state');
      }
    });
  }
  ...
  search(): void {
    ...
    if (this.selectedAmenity()) {
      // Find amenity by name and use its ID for proper filtering
      const amenity = this.amenities().find(a => a.name === this.selectedAmenity());
      if (amenity) {
        query.amenityId = amenity.amenityId;
        query.amenity = amenity.name; // Keep for display
      }
    }
  ...
  searchByAmenity(label: string): void {
    ...
    // Find amenity by name and use its ID for proper filtering
    const amenity = this.amenities().find(a => a.name === label);
    if (amenity) {
      this.router.navigate(['/hotels'], { queryParams: { amenityId: amenity.amenityId, amenity: label } });
    } else {
  ...
  /** Returns only available amenities for a hotel */
  getAvailableHotelAmenities(hotelId: number): HotelAmenityModel[] {
    return this.getHotelAmenities(hotelId).filter(a => a.isAvailable);
  }
