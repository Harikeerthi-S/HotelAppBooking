...
namespace HotelBookingApp.Tests.Services
{
    public class CancellationServiceTests
    {
        ...
        public async Task CreateAsync_ConfirmedBooking_Returns80PercentRefund()
        {
            ...
            Assert.Equal(4000m, result.RefundAmount);
        ...
        {
            _bookingRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(PendingBooking());
            _cancellationRepoMock.Setup(r => r.AddAsync(It.IsAny<Cancellation>()))
                                 .ReturnsAsync((Cancellation c) => { c.CancellationId = 1; return c; });
            _bookingRepoMock.Setup(r => r.UpdateAsync(1, It.IsAny<Booking>())).ReturnsAsync((int _, Booking b) => b);
        ...
        {
            var booking = ConfirmedBooking(); booking.Status = "Cancelled";
            _bookingRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(booking);
            await Assert.ThrowsAsync<BadRequestException>(() =>
                _sut.CreateAsync(new CreateCancellationDto { BookingId = 1, Reason = "X" }));
        }
        ...
        {
            var list = Enumerable.Range(1, 5).Select(i => new Cancellation
            {
                CancellationId = i, BookingId = i, Reason = "R", Status = "Pending",
                CancellationDate = DateTime.UtcNow.AddMinutes(-i)
            }).ToList();
            _cancellationRepoMock.Setup(r => r.GetAllAsync()).ReturnsAsync(list);
        ...
        {
            _cancellationRepoMock.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<Cancellation>());
        ...
        public async Task GetByUserAsync_ReturnsPaged()
        {
            _bookingRepoMock.Setup(r => r.FindAllAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Booking, bool>>>()))
                            .ReturnsAsync(new List<Booking> { new() { BookingId = 1, UserId = 1 } });
            _cancellationRepoMock.Setup(r => r.FindAllAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Cancellation, bool>>>()))
                                 .ReturnsAsync(new List<Cancellation>
                                 {
                                     new() { CancellationId = 1, BookingId = 1, Reason = "R", Status = "Pending", CancellationDate = DateTime.UtcNow }
                                 });
        ...
        [Fact]
        public async Task UpdateStatusAsync_Approved_ReturnsUpdated()
        {
            var c = new Cancellation { CancellationId = 1, BookingId = 1, Status = "Pending", RefundAmount = 4000 };
            _cancellationRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(c);
            _cancellationRepoMock.Setup(r => r.UpdateAsync(1, It.IsAny<Cancellation>())).ReturnsAsync((int _, Cancellation x) => x);
        ...
        [Fact]
        public async Task UpdateStatusAsync_Rejected_ReturnsUpdated()
        {
            var c = new Cancellation { CancellationId = 1, BookingId = 1, Status = "Pending", RefundAmount = 0 };
            _cancellationRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(c);
            _cancellationRepoMock.Setup(r => r.UpdateAsync(1, It.IsAny<Cancellation>())).ReturnsAsync((int _, Cancellation x) => x);

            var result = await _sut.UpdateStatusAsync(1, "Rejected");
            Assert.Equal("Rejected", result.Status);
        }

        [Fact]
        public async Task UpdateStatusAsync_WithRefundAmount_UpdatesRefund()
        {
            var c = new Cancellation { CancellationId = 1, BookingId = 1, Status = "Pending", RefundAmount = 1000 };
            _cancellationRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(c);
            _cancellationRepoMock.Setup(r => r.UpdateAsync(1, It.IsAny<Cancellation>())).ReturnsAsync((int _, Cancellation x) => x);

            var result = await _sut.UpdateStatusAsync(1, "Approved", refundAmount: 3000);
        ...
        {
            var c = new Cancellation { CancellationId = 1, BookingId = 1, Status = "Approved", RefundAmount = 4000 };
            var booking = new Booking { BookingId = 1, UserId = 1, HotelId = 1, RoomId = 1, Status = "Cancelled",
        ...
        [Fact]
        public async Task UpdateStatusAsync_Refunded_BookingNull_StillReturns()
        {
            var c = new Cancellation { CancellationId = 1, BookingId = 99, Status = "Approved", RefundAmount = 4000 };
            _cancellationRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(c);
            _cancellationRepoMock.Setup(r => r.UpdateAsync(1, It.IsAny<Cancellation>())).ReturnsAsync((int _, Cancellation x) => x);
            _bookingRepoMock.Setup(r => r.GetByIdAsync(99)).ReturnsAsync((Booking?)null);
