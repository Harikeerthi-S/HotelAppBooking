...
namespace HotelBookingApp.Tests.Services
{
    public class AuthServiceTests
    {
        ...
        [Fact]
        public async Task LoginAsync_AdminRole_ReturnsAdminRole()
        {
            var hash = new byte[] { 9, 8, 7 };
            var user = new User { UserId = 2, UserName = "Admin", Email = "admin@test.com", Role = "admin", PasswordHash = hash };
            _userRepoMock.Setup(r => r.FindAsync(It.IsAny<System.Linq.Expressions.Expression<Func<User, bool>>>()))
                         .ReturnsAsync(user);
            _passwordMock.Setup(p => p.VerifyPassword(It.IsAny<string>(), hash)).Returns(true);

            var result = await _sut.LoginAsync(new LoginRequestDto { Email = "admin@test.com", Password = "adminpass" });
