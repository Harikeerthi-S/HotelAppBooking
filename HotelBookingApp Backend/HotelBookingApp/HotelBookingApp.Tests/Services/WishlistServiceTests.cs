...
namespace HotelBookingApp.Tests.Services
{
    public class WishlistServiceTests
    {
        ...
        [Fact]
        public async Task AddAsync_HotelNull_ThrowsNotFoundException()
        {
            _userRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(SampleUser());
        ...
        [Fact]
        public async Task AddAsync_HotelInactive_ThrowsNotFoundException()
        {
            _userRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(SampleUser());
        ...
        [Fact]
        public async Task AddAsync_AlreadySaved_ThrowsAlreadyExists()
        {
            _userRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(SampleUser());
            _hotelRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(ActiveHotel());
            _wishlistRepoMock.Setup(r => r.ExistsAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Wishlist, bool>>>()))
        ...
        [Fact]
        public async Task GetByUserAsync_ReturnsOrderedByDate()
        {
            _wishlistRepoMock.Setup(r => r.FindAllAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Wishlist, bool>>>()))
                             ...
                             {
                                 new() { WishlistId = 1, UserId = 1, HotelId = 1, SavedAt = DateTime.UtcNow.AddMinutes(-5) },
                                 new() { WishlistId = 2, UserId = 1, HotelId = 2, SavedAt = DateTime.UtcNow }
                             });

            var result = (await _sut.GetByUserAsync(1)).ToList();
            Assert.Equal(2, result.Count);
            Assert.Equal(2, result[0].HotelId); // newest first
        }
