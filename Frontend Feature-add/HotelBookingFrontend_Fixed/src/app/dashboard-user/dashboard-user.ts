...
type DashTab = 'bookings' | 'payments' | 'cancellations' | 'notifications' | 'reviews' | 'wallet' | 'support';
...
export class DashboardUser implements OnInit, OnDestroy, AfterViewChecked {
  ...
  loadWallet(): void {
    ...
    this.api.apiGetWallet(uid).subscribe({
      next: (w: WalletModel) => { this.wallet.set(w); this.walletLoading.set(false); },
