...
export class APIService {
  ...
  /* ── Reviews ── */
  apiCreateReview(hotelId: number, userId: number, rating: number, comment: string, photoUrl?: string) {
    return this.http.post<ReviewModel>(`${API}/review`, { hotelId, userId, rating, comment, photoUrl });
  }
  apiUploadReviewPhoto(reviewId: number, file: File) {
    const form = new FormData();
    form.append('photo', file);
    return this.http.post<ReviewModel>(`${API}/review/${reviewId}/photo`, form);
  }
  ...
  /* ── Wallet ── */
  apiGetWallet(userId: number) {
    return this.http.get<any>(`${API}/wallet/user/${userId}`);
  }
}
