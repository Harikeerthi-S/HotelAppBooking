    ...
    public class ChatService : IChatService
    {
        ...
        private async Task<(string reply, string intent)> BuildReplyAsync(string msg, int? userId)
        {
            ...
            if (ContainsAny(lower, "book", "booking", "reserve", "reservation", "check-in", "check in", "check-out", "check out", "how to book", "make a booking")
                && !ContainsAny(lower, "check-in time", "check in time", "check-out time", "check out time"))
        ...
        {
            // Uses AppDelegateFactory.StandardRefundPolicy tiered policy
            if (ContainsAny(lower, "calculate refund", "how much refund", "refund amount", "refund calculator", "refund if i cancel"))
                return
                    "🧮 **Refund Calculator:**\n\n" +
                    "Your refund is calculated automatically based on days before check-in:\n\n" +
                    ...
                    "• Cancel 6 days before → ₹5,000 (100%)\n" +
                    "• Cancel 4 days before → ₹2,500 (50%)\n" +
                    "• Cancel 2 days before → ₹1,250 (25%)\n" +
                    "• Cancel same day → ₹0\n\n" +
                    "💡 Refund is credited automatically to your **wallet** on approval.";

            if (ContainsAny(lower, "refund", "money", "get back", "money back"))
                return
                    "💰 **Refund Policy:**\n\n" +
                    "StayEase uses a tiered refund policy based on how early you cancel:\n\n" +
                    "• **≥ 5 days** before check-in → **100% refund**\n" +
                    "• **3–5 days** before check-in → **50% refund**\n" +
                    "• **1–3 days** before check-in → **25% refund**\n" +
                    "• **≤ 1 day** before check-in → **No refund**\n\n" +
                    "Approved refunds are credited directly to your **wallet** — no waiting!\n\n" +
            ...
            if (ContainsAny(lower, "how to cancel", "cancel my", "request cancel", "steps to cancel"))
                return
                    ...
                    "4. Refund is auto-calculated by our tiered policy\n" +
                    "5. Admin approves → refund goes straight to your **wallet**\n\n" +
            ...
            return
                ...
                "• 1–3 days → **25% refund**\n" +
                "• ≤ 1 day  → **No refund**\n\n" +
                "Refunds are credited to your **wallet** automatically on approval.\n\n" +
                "Ask me:\n• **How to cancel** — step-by-step\n• **Calculate refund** — see examples";
        }
