...
namespace HotelBookingApp.Tests.Services
{
    public class HotelAmenityServiceTests
    {
        ...
        private static Amenity SampleAmenity() => new() { AmenityId = 1, Name = "WiFi", Icon = "wifi-icon", Description = "Free WiFi" };
        ...
        public async Task CreateAsync_ValidAssignment_ReturnsDto()
        {
            ...
            _haRepoMock.Setup(r => r.AddAsync(It.IsAny<HotelAmenity>()))
                       .ReturnsAsync((HotelAmenity ha) => { ha.HotelAmenityId = 1; ha.Amenity = SampleAmenity(); return ha; });

            var result = await _sut.CreateAsync(new CreateHotelAmenityDto { HotelId = 1, AmenityId = 1 });
            Assert.Equal(1, result.HotelAmenityId);
            Assert.Equal("WiFi", result.AmenityName);
        }
        ...
        [Fact]
        public async Task CreateAsync_AlreadyAssigned_ThrowsAlreadyExists()
        {
            _hotelRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(ActiveHotel());
            _amenityRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(SampleAmenity());
            _haRepoMock.Setup(r => r.ExistsAsync(It.IsAny<System.Linq.Expressions.Expression<Func<HotelAmenity, bool>>>()))
        ...
        [Fact]
        public async Task GetByIdAsync_WithAmenity_ReturnsDto()
        {
            var ha = new HotelAmenity { HotelAmenityId = 1, HotelId = 1, AmenityId = 1, Amenity = SampleAmenity() };
            _haRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(ha);

            var result = await _sut.GetByIdAsync(1);
            Assert.Equal("WiFi", result!.AmenityName);
            Assert.Equal("wifi-icon", result.AmenityIcon);
            Assert.Equal("Free WiFi", result.AmenityDescription);
        }

        [Fact]
        public async Task GetByIdAsync_WithoutAmenity_ReturnsEmptyName()
        {
            var ha = new HotelAmenity { HotelAmenityId = 1, HotelId = 1, AmenityId = 1 }; // no Amenity nav
            _haRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(ha);

            var result = await _sut.GetByIdAsync(1);
            Assert.Equal(string.Empty, result!.AmenityName);
            Assert.Null(result.AmenityIcon);
            Assert.Null(result.AmenityDescription);
        }
        ...
        public async Task GetAllAsync_ReturnsAll()
        {
            _haRepoMock.Setup(r => r.GetAllIncludingAsync(It.IsAny<System.Linq.Expressions.Expression<Func<HotelAmenity, object>>[]>()))
                       ...
                       {
                           new() { HotelAmenityId = 1, HotelId = 1, AmenityId = 1, Amenity = SampleAmenity() },
