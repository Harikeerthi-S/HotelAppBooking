    ...
    public class ReviewServiceTests
    {
        ...
        public ReviewServiceTests()
        {
            ...
            _sut = new ReviewService(
                ...
                _walletMock.Object,
                _auditMock.Object,
                MockLogger.Create<ReviewService>());
        }
        ...
        // ── CreateAsync — no photo ────────────────────────────────────────

        [Fact]
        public async Task CreateAsync_ValidReview_NoPhoto_ReturnsDto()
        {
        ...
        public async Task CreateAsync_WithPhoto_Credits100Coins()
        {
            ...
            _reviewRepoMock.Setup(r => r.ExistsAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Review, bool>>>()))
                           .ReturnsAsync(false);
            _reviewRepoMock.Setup(r => r.AddAsync(It.IsAny<Review>()))
                           .ReturnsAsync((Review r) => { r.ReviewId = 5; r.PhotoUrl = "/uploads/reviews/test.jpg"; return r; });

            var result = await _sut.CreateAsync(new CreateReviewDto
            {
                HotelId = 1, UserId = 1, Rating = 5, Comment = "Amazing!",
                PhotoUrl = "/uploads/reviews/test.jpg"
            });

            Assert.Equal(100, result.CoinsEarned);
            Assert.Equal("/uploads/reviews/test.jpg", result.PhotoUrl);
            _walletMock.Verify(w => w.CreditAsync(1, 100m, It.IsAny<string>(), 5), Times.Once);
        }

        [Fact]
        public async Task CreateAsync_WithPhoto_WalletFails_StillReturnsReview()
        {
            _hotelRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(ActiveHotel());
            _userRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(SampleUser());
            _reviewRepoMock.Setup(r => r.ExistsAsync(It.IsAny<System.Linq.Expressions.Expression<Func<Review, bool>>>()))
                           .ReturnsAsync(false);
            _reviewRepoMock.Setup(r => r.AddAsync(It.IsAny<Review>()))
                           .ReturnsAsync((Review r) => { r.ReviewId = 6; r.PhotoUrl = "/img.jpg"; return r; });
            _walletMock.Setup(w => w.CreditAsync(It.IsAny<int>(), It.IsAny<decimal>(), It.IsAny<string>(), It.IsAny<int?>()))
                       .ThrowsAsync(new Exception("Wallet down"));

            var result = await _sut.CreateAsync(new CreateReviewDto
            {
                HotelId = 1, UserId = 1, Rating = 4, Comment = "Good", PhotoUrl = "/img.jpg"
            });

            Assert.Equal(6, result.ReviewId); // review still created
        }
        ...
        [Fact]
        public async Task UploadPhotoAsync_NoExistingPhoto_Credits100Coins()
        {
            var review = new Review { ReviewId = 1, HotelId = 1, UserId = 1, Rating = 4, Comment = "Good" };
            _reviewRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(review);
            _reviewRepoMock.Setup(r => r.UpdateAsync(1, It.IsAny<Review>())).ReturnsAsync((int _, Review r) => r);

            var result = await _sut.UploadPhotoAsync(1, "/uploads/reviews/photo.jpg");

            Assert.Equal("/uploads/reviews/photo.jpg", result.PhotoUrl);
            Assert.Equal(100, result.CoinsEarned);
            _walletMock.Verify(w => w.CreditAsync(1, 100m, It.IsAny<string>(), 1), Times.Once);
        }

        [Fact]
        public async Task UploadPhotoAsync_AlreadyHasPhoto_ThrowsBadRequest()
        {
            var review = new Review { ReviewId = 1, HotelId = 1, UserId = 1, Rating = 4, Comment = "Good", PhotoUrl = "/existing.jpg" };
            _reviewRepoMock.Setup(r => r.GetByIdAsync(1)).ReturnsAsync(review);

            await Assert.ThrowsAsync<BadRequestException>(() => _sut.UploadPhotoAsync(1, "/new.jpg"));
        }

        [Fact]
        public async Task UploadPhotoAsync_EmptyUrl_ThrowsBadRequest()
        {
            await Assert.ThrowsAsync<BadRequestException>(() => _sut.UploadPhotoAsync(1, "  "));
        }

        [Fact]
        public async Task UploadPhotoAsync_ReviewNotFound_ThrowsNotFoundException()
        {
            _reviewRepoMock.Setup(r => r.GetByIdAsync(99)).ReturnsAsync((Review?)null);
            await Assert.ThrowsAsync<NotFoundException>(() => _sut.UploadPhotoAsync(99, "/photo.jpg"));
        }
